...
export default async function RadiologyPage() {
  ...
  for (const visit of inProgressVisits) {
    ...
    if (workflow) {
      const radSteps = workflow.workflowSteps.filter(
        (step) => step.department === 'RADIOLOGY' && step.status !== 'DONE'
      )
      if (radSteps.length > 0) {
        radiologyQueue.push({ visit, workflow, radSteps })
      }
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Radiology</h1>

      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">
            Radiology Queue ({radiologyQueue.length})
          </h2>
        </div>
        <div className="divide-y divide-gray-200">
          {radiologyQueue.map(({ visit, workflow, radSteps }) => (
            <div key={visit.id} className="px-6 py-4">
              ...
              <div className="flex space-x-2">
                <a
                  href={`/visits/${visit.id}/results/radiology`}
